...
public:
    ...
    double (*move_eval_function)(Tile& target, int playerId, Tiling* tiling) = nullptr; // Function pointer for the move evaluation function that evalueates the score of the given target tile
    ...
    // Constructor
    Solver(Tiling* tiling, int win_length, bool is_maker_maker, bool is_two_player, double (*move_eval_function)(Tile& target, int playerId, Tiling* tiling), std::vector<Tile*> (*move_selection_function)(Tiling* tiling, int playerId), int max_depth) {}

    // Destructor
    ~Solver() {}

private:
    Tile* selectBestMove(Tiling * tiling, int playerId) {}
